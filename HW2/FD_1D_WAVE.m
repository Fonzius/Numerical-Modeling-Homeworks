...
u  = @(x,t) cos(2*pi*x)*sin(2*pi*t);
ut = @(x,t) 2*pi*cos(2*pi*x)*cos(2*pi*t);
ux = @(x,t) -2*pi*sin(2*pi*x)*sin(2*pi*t);
...
v0  = @(x) 2*pi*cos(2*pi*x); % for system of eq
up0 = @(x) 0.*x;             % for system of eq

% Initial conditions for w~_t = D w~_x
% by using the transformation w~(x,0) = T^(-1) w(x,0)

w0 = @(x) sqrt(1+c^2)/(2*c).*( 2*pi*cos(2*pi*x));
w1 = @(x) sqrt(1+c^2)/(2*c).*( -2*pi*cos(2*pi*x)); 

%boundary conditions
gp1 = @(t) 2*pi*cos(2*pi.*t);
gp2 = @(t) 0.*t;
...
% 
%     W1(j,1) = w0(I(1) + (j-1)*dx);
%     W2(j,1) = w1(I(1) + (j-1)*dx);
% end
% 
% for j = 1 : NX+1
%     U1(j,1) = v0(I(1) + (j-1)*dx);
%     U2(j,1) = up0(I(1) + (j-1)*dx);
% end
...
% 
% for j = 2:NX+2
%     W1(j,1) = sqrt(1+c^2)/(2*c)*(U1(j-1, 1) + c*U2(j-1, 1));
%     W2(j,1) = sqrt(1+c^2)/(2*c)*(-U1(j-1, 1) + c*U2(j-1, 1));
% end
...
% W1(1,1) = W1(2,1);
% W1(end,1) = 2*W1(end-1,1) - W1(end-2,1);
% W2(1,1) = W2(2,1);
% W2(end,1) = 2*W2(end-1,1) - W2(end-2,1);
for j = 1 : NX+1
    SOL_w(j,1)      = w0(I(1) + (j-1)*dx);
    SOL_w(NX+1+j,1) = w1(I(1) + (j-1)*dx);
    SOL_u(j,1)      = v0(I(1) + (j-1)*dx);
...
for n = 1:NT
    for j = 2:NX
        ...
        elseif flag == 3 % LW scheme
            SOL_w(j,n+1) = SOL_w(j,n)  ...
                           - 0.5*lambda*a1*(SOL_w(j+1,n)-SOL_w(j-1,n)) ...
                           + 0.5*lambda^2*a1^2*(SOL_w(j+1,n)-2*SOL_w(j,n)+SOL_w(j-1,n));

            SOL_w(NX+1+j,n+1) = SOL_w(NX+1+j,n) ...
                                - 0.5*lambda*a2*(SOL_w(NX+1+j+1,n)-SOL_w(NX+1+j-1,n)) ...
    ...
    % update w~_2(L,t)
    SOL_w(end,n+1)  = sqrt(1+c^2)/c*gp2((n+1)*dt) - SOL_w(NX+1,n+1);
...
% L2ut_ERR   = norm(SOLut_EX-U1(1:end,n+1),2)*dx^0.5;
% L1ut_ERR   = norm(SOLut_EX-U1(1:end,n+1),1)*dx;
% LINFut_ERR = norm(SOLut_EX-U1(1:end,n+1),Inf);
% 
% L2ux_ERR   = norm(SOLux_EX-U2(1:end,n+1),2)*dx^0.5;
% L1ux_ERR   = norm(SOLux_EX-U2(1:end,n+1),1)*dx;
% LINFux_ERR = norm(SOLux_EX-U2(1:end,n+1),Inf);
% 
% Err_t = [L2ut_ERR, L1ut_ERR, LINFut_ERR]
% Err_x = [L2ux_ERR, L1ux_ERR, LINFux_ERR]
